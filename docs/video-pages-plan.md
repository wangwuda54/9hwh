...
`/v/` 是视频关键词落地页，不是 AI 说明页。页面要像真实官网业务页面：标题直接承接关键词，正文直接讲业务场景、素材用途、落地页承接和联系动作。

写作规则：

- 不要用编号标题，例如“AI数字人视频生成服务 001”。
- 不要写免责声明式文案，例如“不承诺排名、审核或转化结果”。
- 不要堆多个 CTA，详情页主要保留 `Telegram 咨询` 作为咨询按钮。
- Hero 区只放关键词标题和业务简介，不放按钮。
- 视频下方放一次 `Telegram 咨询`，底部联系区可再放一次。
- `videos.json` 管视频资产，`video_topics.json` 管页面内容。
- 修改样式时只改 `.video-*`、`.video-page-*`、`.video-card-*` 等视频页局部 class，不改全站风格。
